...
namespace libmmd
{
	namespace
	{
		...
		bool ReadPMXFile(PMXFile * pmxFile, File& file)
		{
			...
			{
				LIBMMD_ERROR("ReadMorph Fail.");
				return false;
			}

			if (!ReadDisplayFrame(pmxFile, file))
			{
				LIBMMD_ERROR("ReadDisplayFrame Fail.");
				return false;
			}

			if (!ReadRigidbody(pmxFile, file))
			{
				LIBMMD_ERROR("ReadRigidbody Fail.");
				return false;
			}

			if (!ReadJoint(pmxFile, file))
			{
				LIBMMD_ERROR("ReadJoint Fail.");
				return false;
			}
			...
			if (file.Tell() < file.GetSize())
			{
				...
				{
					LIBMMD_ERROR("ReadSoftbody Fail.");
					return false;
				}
			}
		...
		bool WritePMXFile(const PMXFile* pmxFile, File& file)
		{
			...
			{
				LIBMMD_ERROR("WriteTexture Fail.");
				return false;
			}
			...
			{
				LIBMMD_ERROR("WriteMorph Fail.");
				return false;
			}

			if (!WriteDisplayFrame(pmxFile, file))
			{
				LIBMMD_ERROR("WriteDisplayFrame Fail.");
				return false;
			}

			if (!WriteRigidbody(pmxFile, file))
			{
				LIBMMD_ERROR("WriteRigidbody Fail.");
				return false;
			}

			if (!WriteJoint(pmxFile, file))
			{
				LIBMMD_ERROR("WriteJoint Fail.");
				return false;
			}
	...
	bool ReadPMXFile(PMXFile * pmxFile, const char* filename)
	{
		...
		{
			LIBMMD_INFO("PMX File Open Fail. {}", filename);
			return false;
		}
	...
	bool WritePMXFile(const PMXFile* pmxFile, const char* filename)
	{
		...
		{
			LIBMMD_INFO("PMX File Write Fail. {}", filename);
			return false;
		}
		LIBMMD_INFO("PMX File Write Successed. {}", filename);
