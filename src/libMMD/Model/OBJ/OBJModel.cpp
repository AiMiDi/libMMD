...
namespace libmmd
{
	namespace
	{
		...
		public:
				...
				) override
			{
				...
				if (!fr.Open(mtlPath))
				{
					LIBMMD_WARN("Failed to open MTL file.");
					LIBMMD_INFO("Try obj name + .mtl.");
	...
	{
		LIBMMD_INFO("Open OBJ file. {}", filepath);

		TextFileReader textFileReader;
		if (!textFileReader.Open(filepath))
		{
			LIBMMD_WARN("Failed to open OBJ file. {}", filepath);
			return false;
		}
		...
		if (!ret)
		{
			LIBMMD_WARN("Failed to load OBJ file. {}", filepath);
			return false;
		}
		...
		for (const auto& shape : shapes)
		{
			...
			for (size_t faceIdx = 0; faceIdx < shape.mesh.num_face_vertices.size(); faceIdx++)
			{
				...
				{
					LIBMMD_WARN("[num_face_vertices] != 3");
					LIBMMD_WARN("OBJ File Fail. {}", filepath);
					return false;
				}
		...
		LIBMMD_INFO("OBJ File Success. {}", filepath);
		return true;
	}
