...
namespace libmmd
{
	bool XFileModel::Load(const char* filepath)
	{
		...
		{
			LIBMMD_WARN("Failed to open file.[{}]", filepath);
			return false;
		}
		...
		{
			LIBMMD_WARN("Failed to open file.[{}]", filepath);
			LIBMMD_WARN("XFileLoader message : {}", loader.GetErrorMessage());
			return false;
		}
		...
		for (const auto& xfileFrame : xfile.m_frames)
		{
			...
			if (xfileFrame->m_mesh.m_positions.size() != 0)
			{
				...
				{
					LIBMMD_ERROR("XFile face size error.");
					return false;
				}
				...
				{
					LIBMMD_ERROR("XFile uv size error.");
					return false;
				}
