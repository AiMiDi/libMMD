...
namespace libmmd
{
	bool VMDLightAnimation::read_from_file(const file& file)
	{
		if (!file.read_elements(frame_num_))
			return false;
		if (!file.read_elements(m_data->light_color))
			return false;
		if (!file.read_elements(m_data->light_position))
			return false;
		return true;
	}
	...
	bool VMDLightAnimation::write_to_file(const file& file) const
	{
		if (!file.write_elements(frame_num_))
			return false;
		if (!file.write_elements(m_data->light_color))
			return false;
		if (!file.write_elements(m_data->light_position))
			return false;
		return true;
	}
}
