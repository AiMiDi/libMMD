...
namespace libmmd
{
	bool VMDMorphAnimation::read_from_file(const file& file)
	{
		...
		if (!file.read_elements(m_data->weight))
			return false;
		m_data->morph_name = code_converter::shift_jis_to_utf8(morph_name);
		return true;
	}

	bool VMDMorphAnimation::write_to_file(const file& file) const
	{
		std::string morph_name = code_converter::utf8_to_shift_jis(m_data->morph_name);
		morph_name.resize(15, '\0');
		if (!file.write_elements(morph_name.c_str(), 15LLU))
			return false;
		if (!file.write_elements(frame_num_))
			return false;
		if (!file.write_elements(m_data->weight))
			return false;
		return true;
	}
}