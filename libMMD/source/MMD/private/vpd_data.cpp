...
namespace libmmd
{
	bool VPDFile::load_from_file(const path& path) {
		file file;
		...
		{
			MessageDialog("VPD File Format Error.");
			return false;
		}
		/* remove comment */
		for (auto& line : lines) {
		...
		{
			auto& [name, translate, rotation] = this->m_bones.emplace_back();
	...
	bool VPDFile::save_to_file(const path& path) const
	{
