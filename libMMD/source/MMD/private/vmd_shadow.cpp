...
namespace libmmd
{
	bool VMDShadow::read_from_file(const file& file)
	{
		if (!file.read_elements(frame_num_))
			return false;
		if (!file.read_elements(m_data->shadow_type))
			return false;
		if (!file.read_elements(m_data->distance))
			return false;
		return true;
	}
	...
	bool VMDShadow::write_to_file(const file& file) const
	{
		if (!file.write_elements(frame_num_))
			return false;
		if (!file.write_elements(m_data->shadow_type))
			return false;
		if (!file.write_elements(m_data->distance))
			return false;
		return true;
	}
}